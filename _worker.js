			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				switch (url.pathname) {
					...
					case `/bestip/geo`: {
						...
						const url = `https://sub.xf.free.hr/auto?host=${request.headers.get('Host')}&uuid=${userID}&path=/vless`;
...
function getVLESSConfig(userIDs, hostName) {
	...
	const output = userIDArray.map((userID) => {
		...
		return `<b>UUID: ${userID}</b><br>${hashSeparator}</br>\n<b><green>🇮🇩 V2RAY DEFAULT IP 🇮🇩</green></b>
--------------------------
${vlessMain}
<button onclick='copyToClipboard("${vlessMain}")'><i class="fa fa-clipboard"></i> Copy Vless Default IP</button>
--------------------------
<b><green>🇮🇩 V2RAY WITH BEST IP 🇮🇩</green></b>
--------------------------
${vlessSec}
<button onclick='copyToClipboard("${vlessSec}")'><i class="fa fa-clipboard"></i> Copy Vless Best IP</button>
--------------------------`;
	}).join('\n');
...
<a href='v2rayng://install-config?url=${encodeURIComponent(subbestip)}' target='_blank'>v2rayNG</a></p>`;
  ...
  <head>
	...
	<meta name='twitter:image' content='https://github.com/jaka1m/project/assets/61670480/b34667e7-1752-43be-98cf-401f86d5887c' />
...
function createVLESSSub(userID_Path, hostName) {
	...
	const commonUrlPart_http = `?encryption=none&security=none&fp=random&type=ws&host=${hostName}&path=%2Fvless#`;
	const commonUrlPart_https = `?encryption=none&security=tls&sni=${hostName}&fp=random&type=ws&host=${hostName}&path=%2Fvless#`;
